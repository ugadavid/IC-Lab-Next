...
async function loadMockAnalysis() {
  textContainer.textContent = "Chargement du paquet JSON de démonstration…";
  globalFeedback.textContent = "Connexion au mock Dico-IC…";
  ...
  try {
    ...
    analysisPackage = validateAnalysisPackage(await response.json());
    updateLanguageLabel();
...
function showMockLoadError(error) {
  ...
  textContainer.textContent =
    "Impossible de charger le mock JSON. Servez ce dossier avec un serveur HTTP local, puis rechargez la page.";
  ...
  transformationsBox.textContent = "Le prototype historique reste disponible dans index-0.0.8.2.html.";
...
function updateSidebar() {
  ...
  if (selectedTokens.length === 0) {
    ...
    globalFeedback.textContent = `Mock JSON chargé · ${analysisPackage.tokens.length} tokens · ${warningCount} avertissement(s).`;
...
function updateTransformationsBox() {
  ...
  transformationsBox.textContent = rows.length
    ? rows.join("\n")
    : "Aucun enrichissement reçu pour ce tamis dans le mock.";
}
