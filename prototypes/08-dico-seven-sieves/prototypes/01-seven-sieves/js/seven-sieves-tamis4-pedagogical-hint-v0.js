...
const pedagogicalHints = Object.freeze([
  Object.freeze({
    sieve: 4,
    language: "es",
    pattern: "ñ",
    ...
    examples: Object.freeze(["España / Espagne"]),
    caution: "Indice utile, mais pas une règle universelle."
  }),
  Object.freeze({
    sieve: 4,
    language: "es",
    ...
    matchMode: "suffix",
    comparisonHint: "tion",
    title: "Indice -ción / -tion",
    message: "Les noms espagnols terminés par -ción correspondent souvent à des noms français en -tion.",
    ...
    caution: "Indice fréquent mais non systématique."
  }),
  Object.freeze({
    sieve: 4,
    language: "es",
    ...
    comparisonHint: "té",
    title: "Indice -dad / -té",
    message: "Les noms espagnols terminés par -dad correspondent souvent à des noms français en -té.",
    examples: Object.freeze([
  ...
  Object.freeze({
    ...
    matchMode: "suffix",
    comparisonHint: "ment",
    title: "Indice -mente / -ment",
    message: "Les adverbes espagnols en -mente correspondent souvent aux adverbes français en -ment.",
    examples: Object.freeze([
...
const connectorHelpButton = document.getElementById("connectorHelpButton");
const connectorHelpStatus = document.getElementById("connectorHelpStatus");
...
function getConnectorHelpMatches() {
  return (analysisPackage?.pedagogical_enrichments || [])
    .filter(item => item.type === "connector_help"
      && item.source?.kind === "connector_help"
      && Array.isArray(item.token_indexes))
    .map(item => ({ help: item, tokenIndexes: item.token_indexes }));
}

function getConnectorHelpMatchesForToken(tokenIndex) {
  return getConnectorHelpMatches().filter(match => match.tokenIndexes.includes(tokenIndex));
}

function formatConnectorHelp(match) {
  return [
    `⚡ ${match.help.label || "Connecteur logique"}`,
    "",
    "Fonction :",
    match.help.function,
    "",
    match.help.pedagogical_hint,
    "",
    "Expression repérée :",
    match.help.expression,
    match.help.example ? `\nExemple :\n${match.help.example}` : "",
    "",
    "Prudence :",
    match.help.caution || "La fonction exacte peut dépendre du contexte."
  ].filter(value => value !== "").join("\n");
}

function getLocalPedagogicalEnrichments(token, sieveId = null) {
  if (!analysisPackage || (sieveId !== null && sieveId !== 4)) return [];

  const analyzedLanguage = analysisPackage.languages?.source || sourceLanguage.value;
  const surface = token.surface.toLocaleLowerCase(analyzedLanguage);

  return pedagogicalHints
    .filter(hint => hint.sieve === 4
      && hint.language === analyzedLanguage
      && (hint.matchMode === "suffix"
        ? surface.endsWith(hint.pattern)
        : surface.includes(hint.pattern)))
    .map(hint => ({
      id: `local-t4-${token.index}-${hint.pattern}`,
      sieve_id: hint.sieve,
      type: "grapho_phonetic_signal",
      label: hint.title,
      explanation: `${hint.message}\n\nExemple : ${hint.examples.join(" ; ")}.`,
      caution: hint.caution,
      confidence: 0.75,
      level: "informational",
      source: {
        kind: "heuristic",
        label: "Prototype pédagogique local Seven Sieves"
      },
      payload: {
        pattern: hint.pattern,
        scope: hint.matchMode === "suffix" ? "suffix" : "contains",
        comparison_hint: hint.comparisonHint || "gn"
      }
    }));
}
...
function getTokenEnrichments(tokenIndex, sieveId = null) {
  ...
  const apiEnrichments = sieveId === null
    ? token.enrichments
    : token.enrichments.filter(item => item.sieve_id === sieveId);
  const localEnrichments = getLocalPedagogicalEnrichments(token, sieveId)
    .filter(local => !apiEnrichments.some(api => api.sieve_id === local.sieve_id
      && api.type === local.type
      && api.payload?.pattern === local.payload.pattern));

  return [...apiEnrichments, ...localEnrichments];
}
...
function validateAnalysisPackage(data) {
  ...
  if (data.pedagogical_enrichments === undefined) {
    data.pedagogical_enrichments = [];
  }
  if (!Array.isArray(data.pedagogical_enrichments)) {
    throw new Error("Les enrichissements pédagogiques sont invalides.");
  }
...
function createInteractiveToken(token) {
  ...
  span.addEventListener("mouseenter", () => {
    ...
    const connectorMatches = connectorHelpVisible
      ? getConnectorHelpMatchesForToken(token.index)
      : [];
    const tooltipParts = [
      ...enrichments.map(formatEnrichmentPlain),
      ...connectorMatches.map(formatConnectorHelp)
    ];
...
function isValidatedPluralEnrichment(enrichment) {
  return enrichment?.sieve_id === 6
    && enrichment.type === "morphosyntactic_signal"
    && enrichment.source?.kind === "inflected_form"
    && enrichment.payload?.category === "validated_plural"
    && enrichment.payload?.grammatical_number === "PLURAL"
    && typeof enrichment.payload?.lemma === "string"
    && typeof enrichment.payload?.surface_form === "string";
}

function formatValidatedPluralEnrichment(enrichment) {
  const { lemma, surface_form: surfaceForm } = enrichment.payload;
  return [
    "Indice de pluriel",
    "",
    `Cette forme est le pluriel validé de « ${lemma} ».`,
    "",
    "Singulier :",
    lemma,
    "",
    ...
    "",
    "Information utile :",
    `Le texte emploie « ${lemma} » au pluriel.`,
    "",
    "Prudence :",
    enrichment.caution || "Cette information provient d’un mapping validé dans Dico-IC."
  ].join("\n");
}

function formatEnrichmentPlain(enrichment) {
  if (isValidatedPluralEnrichment(enrichment)) {
    return formatValidatedPluralEnrichment(enrichment);
  }
...
function renderInspectionBox() {
  ...
  const connectorHtml = connectorMatches.length
    ? connectorMatches
        .map(match => `<div class="multi-line">${escapeHtml(formatConnectorHelp(match)).replaceAll("\n", "<br>")}</div>`)
        .join("<hr>")
    : `<div class="multi-line">Aucune aide discursive locale pour cette occurrence.</div>`;
  ...
  inspectionBox.innerHTML = `
    ...
    <div class="inspect-section">
      <h4>Aide discursive expérimentale</h4>
      ${connectorHtml}
    </div>`;
}
...
function updateConnectorHelpControl() {
  connectorHelpButton.setAttribute("aria-pressed", String(connectorHelpVisible));
  connectorHelpButton.textContent = connectorHelpVisible ? "Désactiver" : "Activer";
  connectorHelpStatus.textContent = connectorHelpVisible
    ? "Aides Dico-IC actives · aucun numéro de tamis"
    : "Aides Dico-IC masquées · aucun numéro de tamis";
}

function toggleConnectorHelp() {
  connectorHelpVisible = !connectorHelpVisible;
  updateConnectorHelpControl();
  applyHintVisuals();
  renderInspectionBox();
}
...
connectorHelpButton.addEventListener("click", toggleConnectorHelp);
