-- Draft consolidated procedures for IC-Dico.
-- Source: database/procedures.sql, adjusted for portability.
-- Changes in this draft:
-- - removed user-specific creation clauses;
-- - added DROP PROCEDURE IF EXISTS before each CREATE;
-- - aligned sp_upsert_lexical_form with the 7-parameter version using p_notes.
...
DROP PROCEDURE IF EXISTS sp_get_cognates $$
CREATE PROCEDURE sp_get_cognates (
    IN p_lemma VARCHAR(255)
)
BEGIN
    SELECT
...
DROP PROCEDURE IF EXISTS sp_get_false_friends $$
CREATE PROCEDURE sp_get_false_friends (
    IN p_lemma VARCHAR(255)
)
BEGIN
    SELECT
...
DROP PROCEDURE IF EXISTS sp_get_similarity $$
CREATE PROCEDURE sp_get_similarity (
