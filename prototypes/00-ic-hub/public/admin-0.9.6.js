...
const openProto06SandboxButton = document.querySelector("#openProto06SandboxButton");
const proto06SandboxStatus = document.querySelector("#proto06SandboxStatus");
...
function setProto06SandboxStatus(text, kind = "") {
  if (!proto06SandboxStatus) return;
  proto06SandboxStatus.textContent = text;
  proto06SandboxStatus.className = `sandbox-launch-status ${kind}`.trim();
}
...
openProto06SandboxButton?.addEventListener("click", async () => {
  try {
    openProto06SandboxButton.disabled = true;
    setProto06SandboxStatus("Generation d'un lien sandbox temporaire...", "");
    const result = await api("/api/admin/proto06/sandbox/launch", { method: "POST" });
    setProto06SandboxStatus("Lien sandbox genere. Ouverture dans un nouvel onglet.", "ok");
    const opened = window.open(result.launchUrl, "_blank", "noopener,noreferrer");
    if (!opened) {
      setProto06SandboxStatus("Lien sandbox genere, mais le navigateur a bloque l'ouverture du nouvel onglet.", "error");
    }
  } catch (error) {
    const messageText = String(error.message || "");
    if (messageText.includes("401") || messageText.toLowerCase().includes("connexion")) {
      setProto06SandboxStatus("Connexion admin requise.", "error");
    } else if (messageText.includes("403") || messageText.toLowerCase().includes("refuse")) {
      setProto06SandboxStatus("Acces reserve aux administrateurs.", "error");
    } else {
      setProto06SandboxStatus("Impossible de generer le lien sandbox.", "error");
    }
  } finally {
    openProto06SandboxButton.disabled = false;
  }
});
