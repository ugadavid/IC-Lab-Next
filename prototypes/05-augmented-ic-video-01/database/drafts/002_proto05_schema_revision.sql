-- Proto05 MariaDB relational model -- installable revision 002.
-- Mission 129, 2026-07-27.
--
...
CREATE TABLE media_assets (
  ...
  family_root_asset_id  VARCHAR(191) NOT NULL,
  ...
  CONSTRAINT chk_media_asset_lineage_shape
    CHECK (
      (parent_asset_id IS NULL AND family_root_asset_id = id)
      OR (
...
BEGIN
  ...
  VALUES (
    p_asset_id, p_asset_id, TRIM(p_title), 'active', p_provenance_json
  );
...
BEGIN
  ...
  SELECT family_root_asset_id INTO v_family_root_id
  FROM media_assets
...
BEGIN
  ...
  SELECT status, output_asset_id
    INTO v_current_status, v_output_asset_id
  FROM media_treatments
  WHERE id = p_treatment_id
  FOR UPDATE;
  ...
  IF v_current_status NOT IN ('queued', 'running', 'cancelling')
     AND NOT EXISTS (
       SELECT 1
       FROM media_treatments
       WHERE id = p_treatment_id
         AND status <=> p_status
         AND progress <=> p_progress
         AND diagnostics_json <=> p_diagnostics_json
         AND error_json <=> p_error_json
     ) THEN
    SIGNAL SQLSTATE '45000'
...
BEGIN
  ...
  IF v_status = 'completed' THEN
    IF v_reserved_output_asset_id <> p_output_asset_id
       OR v_existing_output_playable_id <> p_output_playable_id THEN
      SIGNAL SQLSTATE '45000'
        SET MYSQL_ERRNO = 30323,
            MESSAGE_TEXT = 'sp_media_treatment_complete: completed output mismatch';
    END IF;
  ELSE
...
BEGIN
  ...
  UPDATE media_assets
  SET id = id
  WHERE family_root_asset_id = p_family_root_asset_id
    AND deleted_at IS NULL;
      ...
      AND (
        EXISTS (
          ...
          WHERE source_asset.id = t.source_asset_id
            AND source_asset.family_root_asset_id = p_family_root_asset_id
        )
        OR EXISTS (
          SELECT 1
          FROM media_assets output_asset
          WHERE output_asset.id = t.output_asset_id
            AND output_asset.family_root_asset_id = p_family_root_asset_id
        )
      )
  ) THEN
    SIGNAL SQLSTATE '45000'
...
BEGIN
  ...
  SELECT l.id AS link_id, l.role, l.sort_order,
         l.media_asset_id, ma.title AS media_title,
         l.media_playable_id, mp.kind AS playable_kind,
         mp.provider, mp.location_url, mp.embed_video_id,
         mp.availability, pm.duration_ms, pm.mime_type
  FROM activity_media_links l
  INNER JOIN activities a ON a.id = l.activity_id
  INNER JOIN media_assets ma ON ma.id = l.media_asset_id
  INNER JOIN media_playables mp ON mp.id = l.media_playable_id
  ...
  SELECT al.language_id, al.local_label, al.sort_order
  ...
  SELECT s.id AS speaker_id, s.label, s.sort_order
  ...
  SELECT s.id AS segment_id, s.start_ms, s.end_ms, s.text, s.sort_order
  ...
  SELECT ss.segment_id, ss.speaker_id
  ...
  SELECT sl.segment_id, sl.language_id
  ...
  SELECT li.id AS interval_id, li.segment_id, li.language_id,
  ...
  SELECT l.id AS layer_id, l.label, l.description, l.color,
  ...
  SELECT p.id AS phenomenon_id, p.segment_id, p.layer_id,
  ...
  SELECT o.id AS overlay_id, o.annotation_id,
  ...
  SELECT ol.overlay_id, ol.layer_id
