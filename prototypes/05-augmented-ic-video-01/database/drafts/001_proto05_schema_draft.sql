-- Proto05 MariaDB relational model -- DBA review draft 001.
-- Mission 127, 2026-07-26.
--
-- DESIGN ONLY: DO NOT EXECUTE AS A MIGRATION.
-- This file intentionally contains the theoretical database creation so that
-- the complete contract can be reviewed. It creates no user and contains no
-- credential. Binary media remain outside MariaDB.
--
-- Hard decisions represented here:
--   * database name: ic_proto05;
...
CREATE DATABASE IF NOT EXISTS ic_proto05
...
CREATE TABLE import_runs (
  ...
  CONSTRAINT uq_import_source_digest UNIQUE (source_kind, source_digest),
...
CREATE TABLE activities (
  ...
  authoring_revision        BIGINT UNSIGNED NOT NULL DEFAULT 1,
...
CREATE TABLE activity_pedagogical_identities (
  ...
  CONSTRAINT chk_pedagogical_qualified_values
    CHECK (
      (
        intention_state NOT IN ('known', 'to-verify', 'not-applicable')
        OR CHAR_LENGTH(TRIM(intention_value)) > 0
      )
      AND (
        audience_state NOT IN ('known', 'to-verify', 'not-applicable')
        OR CHAR_LENGTH(TRIM(audience_value)) > 0
      )
      AND (
        use_context_state NOT IN ('known', 'to-verify', 'not-applicable')
        OR CHAR_LENGTH(TRIM(use_context_value)) > 0
      )
      AND (
        resource_nature_state <> 'known'
        OR resource_nature_value IS NOT NULL
      )
      AND (
...
CREATE TABLE media_assets (
  ...
  CONSTRAINT fk_media_asset_folder
    ...
    ON DELETE SET NULL,
  CONSTRAINT fk_media_asset_parent
...
CREATE TABLE media_playables (
  ...
  CONSTRAINT chk_media_playable_location
    CHECK (
      (kind = 'local-file' AND storage_scope IS NOT NULL AND storage_key IS NOT NULL)
      OR
      (kind <> 'local-file' AND (location_url IS NOT NULL OR embed_video_id IS NOT NULL))
    ),
  ...
  CONSTRAINT fk_media_playable_asset
    FOREIGN KEY (asset_id) REFERENCES media_assets(id)
    ON DELETE RESTRICT,
  CONSTRAINT fk_media_playable_source
...
CREATE TABLE media_treatments (
  ...
  CONSTRAINT chk_media_treatment_completed_output
    CHECK (
      status <> 'completed'
      OR (output_asset_id IS NOT NULL AND output_playable_id IS NOT NULL)
    ),
  ...
  CONSTRAINT fk_treatment_published_playable
    FOREIGN KEY (published_playable_id) REFERENCES media_playables(id)
    ON DELETE RESTRICT
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;
...
BEGIN
  ...
  FROM activities
  WHERE id = p_source_activity_id AND deleted_at IS NULL
  LIMIT 1
  FOR UPDATE;
  ...
  IF v_exists = 0 THEN
    SIGNAL SQLSTATE '45000'
      SET MYSQL_ERRNO = 30206,
          MESSAGE_TEXT = 'sp_activity_duplicate: source activity not found';
  END IF;
  ...
  INSERT INTO activities (
    ...
    pedagogical_question, authoring_revision, allow_learner_toggle
  )
  SELECT
    p_new_activity_id,
  ...
  INSERT INTO activity_media_links
    (activity_id, role, media_asset_id, media_playable_id)
  SELECT p_new_activity_id, role, media_asset_id, media_playable_id
  FROM activity_media_links
  WHERE activity_id = p_source_activity_id;

  COMMIT;

  SELECT id, folder_id, version, status, title, authoring_revision,
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id = p_activity_id
      ...
      AND authoring_revision = p_expected_revision
    FOR UPDATE
  ) THEN
    SIGNAL SQLSTATE '45000'
      SET MYSQL_ERRNO = 30208,
          MESSAGE_TEXT = 'sp_activity_delete: not found or revision conflict';
  END IF;
  ...
  SELECT id, status, authoring_revision, deleted_at
  FROM activities
  WHERE id = p_activity_id;
END$$
...
BEGIN
  ...
  VALUES (
    p_activity_id, 'primary', p_media_asset_id, p_media_playable_id
  )
  ON DUPLICATE KEY UPDATE
    media_asset_id = VALUES(media_asset_id),
    media_playable_id = VALUES(media_playable_id);
  ...
  SELECT activity_id, role, media_asset_id, media_playable_id, updated_at
  FROM activity_media_links
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    SIGNAL SQLSTATE '45000'
      SET MYSQL_ERRNO = 30212,
          MESSAGE_TEXT = 'sp_activity_replace_authoring: not found or revision conflict';
  END IF;
  ...
  SELECT id, authoring_revision, updated_at
  FROM activities
  WHERE id = p_activity_id;
END$$
...
BEGIN
  ...
  VALUES (p_activity_id, '0.1');

  SELECT id, version, status, title, description, instruction,
         pedagogical_question, authoring_revision, created_at, updated_at
  FROM activities
  WHERE id = p_activity_id;
END$$
  ...
  IF ROW_COUNT() = 0 THEN
    SIGNAL SQLSTATE '45000'
      SET MYSQL_ERRNO = 30203,
          MESSAGE_TEXT = 'sp_activity_update_metadata: not found or revision conflict';
  END IF;
...
BEGIN
  ...
  IF p_lineage_state = 'known'
     ...
     AND (p_parent_activity_id IS NULL OR p_root_activity_id IS NULL) THEN
    SIGNAL SQLSTATE '45000'
  ...
  IF NOT EXISTS (
    ...
    SIGNAL SQLSTATE '45000'
      SET MYSQL_ERRNO = 30219,
          MESSAGE_TEXT = 'sp_activity_replace_pedagogical_details: not found or revision conflict';
  END IF;
...
BEGIN
  ...
  INSERT INTO media_assets (
    id, title, lifecycle, provenance_json
  )
  VALUES (
    p_asset_id, TRIM(p_title), 'active', p_provenance_json
  );

  UPDATE media_assets
  SET family_root_asset_id = p_asset_id
  WHERE id = p_asset_id;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM media_sources
    WHERE id = p_source_id AND asset_id = p_asset_id
    FOR UPDATE
  ) THEN
    SIGNAL SQLSTATE '45000'
...
BEGIN
  ...
  SELECT id, status, progress, source_asset_id, source_playable_id, started_at
  ...
  IF ROW_COUNT() = 0 THEN
    SIGNAL SQLSTATE '45000'
      SET MYSQL_ERRNO = 30305,
          MESSAGE_TEXT = 'sp_media_treatment_update: treatment not mutable';
  END IF;
...
BEGIN
  ...
  SELECT source_asset_id, source_playable_id
    INTO v_source_asset_id, v_source_playable_id
  FROM media_treatments
  WHERE id = p_treatment_id AND status IN ('queued', 'running')
  FOR UPDATE;

  IF v_source_asset_id IS NULL THEN
    SIGNAL SQLSTATE '45000'
      SET MYSQL_ERRNO = 30306,
          MESSAGE_TEXT = 'sp_media_treatment_complete: treatment not completable';
  END IF;

  SELECT COALESCE(family_root_asset_id, id)
    INTO v_family_root_id
  FROM media_assets
  ...
  VALUES (
    p_output_asset_id, v_source_asset_id, v_family_root_id,
    TRIM(p_title), 'active', p_derivation_type
  );
  ...
  UPDATE media_assets
  SET default_playable_id = p_output_playable_id
...
BEGIN
  ...
  WHERE p.id = p_playable_id
    ...
    AND (
      ...
      OR a.default_playable_id IS NULL
      OR a.default_playable_id <> p.id
    )
  FOR UPDATE;
...
BEGIN
  ...
  SELECT playable_id INTO v_playable_id
  FROM storage_operations
  WHERE id = p_operation_id AND status = 'requested'
  FOR UPDATE;

  IF v_playable_id IS NULL THEN
    SIGNAL SQLSTATE '45000'
      SET MYSQL_ERRNO = 30309,
          MESSAGE_TEXT = 'sp_storage_file_removal_complete: request not open';
  END IF;
  ...
  SELECT id, playable_id, status, completed_at
  FROM storage_operations
  WHERE id = p_operation_id;
END$$
  ...
  IF ROW_COUNT() = 0 THEN
    SIGNAL SQLSTATE '45000'
      SET MYSQL_ERRNO = 30310,
          MESSAGE_TEXT = 'sp_storage_file_removal_fail: request not open';
  END IF;
...
BEGIN
  ...
  IF EXISTS (
    ...
    INNER JOIN media_assets m ON m.id = t.source_asset_id
    WHERE COALESCE(m.family_root_asset_id, m.id) = p_family_root_asset_id
...
BEGIN
  SELECT
    ...
    a.pedagogical_question, a.authoring_revision,
    CASE
    ...
    ma.title AS media_title, mp.availability AS media_availability,
...
BEGIN
  ...
  SELECT * FROM activity_pedagogical_text_fields
  WHERE activity_id = p_activity_id ORDER BY field_key;
  SELECT * FROM activity_pedagogical_qualifications
  WHERE activity_id = p_activity_id ORDER BY validated_at, id;
  SELECT * FROM activity_languages
  WHERE activity_id = p_activity_id ORDER BY sort_order, language_id;
  SELECT * FROM activity_transcriptions
  WHERE activity_id = p_activity_id;
  SELECT * FROM activity_speakers
  ...
  SELECT * FROM activity_segment_speakers
  WHERE activity_id = p_activity_id ORDER BY segment_id, speaker_id;
  SELECT * FROM activity_segment_languages
  WHERE activity_id = p_activity_id ORDER BY segment_id, language_id;
  SELECT * FROM activity_language_intervals
  WHERE activity_id = p_activity_id ORDER BY start_ms, sort_order, id;
  SELECT * FROM activity_layers
  WHERE activity_id = p_activity_id ORDER BY sort_order, id;
  SELECT * FROM activity_layer_visibility
  WHERE activity_id = p_activity_id ORDER BY audience, layer_id;
  SELECT * FROM activity_phenomena
  WHERE activity_id = p_activity_id ORDER BY start_ms, sort_order, id;
  SELECT * FROM activity_annotations
  WHERE activity_id = p_activity_id ORDER BY sort_order, id;
  SELECT * FROM activity_overlays
  WHERE activity_id = p_activity_id ORDER BY start_ms, sort_order, id;
  SELECT * FROM activity_overlay_layers
...
BEGIN
  ...
  SELECT p.*, m.analysis_status, m.mime_type, m.duration_ms,
  ...
  SELECT l.activity_id, l.role, a.title, a.status
  FROM activity_media_links l
  INNER JOIN activities a ON a.id = l.activity_id
  WHERE l.media_asset_id = p_asset_id AND a.deleted_at IS NULL
...
BEGIN
  SELECT
    a.id, a.version, a.title, a.description, a.instruction,
    a.pedagogical_question, a.allow_learner_toggle,
    aml.media_asset_id, aml.media_playable_id,
    mp.kind AS playable_kind, mp.provider, mp.location_url,
    mp.embed_video_id, mp.availability,
    pm.duration_ms, pm.mime_type
  FROM activities a
  INNER JOIN activity_media_links aml
    ON aml.activity_id = a.id AND aml.role = 'primary'
  INNER JOIN media_playables mp
    ON mp.id = aml.media_playable_id
   AND mp.asset_id = aml.media_asset_id
  ...
  SELECT s.id, s.start_ms, s.end_ms, s.text, s.sort_order
  ...
  SELECT l.id, l.label, l.description, l.color,
  ...
  SELECT p.* FROM activity_phenomena p
  ...
  SELECT o.* FROM activity_overlays o
